...
import {
  MapPin, BedDouble, Bath, Maximize2, ArrowLeft, ChevronLeft, ChevronRight,
...
function needsGuarantorStep(job: string, criteria: Property['criteria']): boolean {
  ...
  // Contrato sem termo ou a prazo em PT = não precisa perguntar fiador separadamente
...
function calcScore(data: LeadData, property: Property): number {
  ...
  // Compare against property minIncome, not just rent ratio
  ...
  // Extra penalty if income doesn't meet minimum
  ...
  else score = Math.max(0, score - 10); // Over limit: penalty

  // Pets fit
  if (!data.hasPets) score += 10;
  else if (criteria.petsAllowed) score += 10;
  else score = Math.max(0, score - 15); // Pets not allowed: big penalty

  // ── Guarantor fit ────────────────────────────────────────────────────────
  if (criteria.guarantorRequired) {
    if (data.hasGuarantor === 'Sim') score += 8;
    else if (data.hasGuarantor === 'Não' && criteria.advanceWithoutGuarantor) score += 3; // Has fallback values
...
// Step list is dynamic — 'guarantor' is inserted after 'job' when needed
...
const LeadForm = ({ property, onClose }: { property: Property; onClose: () => void }) => {
  ...
  // Dynamic step list based on property criteria + user's job type
                ...
                <div className="p-4 rounded-xl bg-muted/50 border space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-bold">€{(advance * rent).toLocaleString('pt-PT')}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">Caução ({deposit} meses)</span>
                      <span className="font-bold">€{(deposit * rent).toLocaleString('pt-PT')}</span>
                    </div>
                    <div className="border-t pt-2 flex items-center justify-between text-sm font-bold">
                      <span>Total estimado</span>
                      <span>€{((advance + deposit) * rent).toLocaleString('pt-PT')}</span>
                    </div>
                  </div>
        ...
        <div className="grid lg:grid-cols-[1fr,360px] gap-10">
          ...
          <div>
            ...
            >
              {property.images && property.images.length > 0 ? (
                ...
                <>
                  ...
                  />
                  {/* Heavy overlay so brand is readable */}
                  <div className="absolute inset-0 bg-black/55 backdrop-blur-[1px]" />
                  {/* Brand centered */}
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-center">
                      <span className="font-display text-4xl md:text-5xl font-700 tracking-tight text-white/70 leading-none">
                        vyllad
                      </span>
                      <span className="font-display text-4xl md:text-5xl font-700 tracking-tight text-accent/80 leading-none">.</span>
                    </div>
                  </div>
                  {/* Illustrative badge bottom-right */}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
              ...
              {/* Specs row — only rendered when at least one spec exists */}
